...
function MeasureInput({
    ...
    return (
        <label className="grid gap-1.5 text-xs font-semibold text-slate-400">
            {label}
            <span className="flex items-center rounded-lg border border-white/10 bg-[#111722] pr-3 focus-within:border-[#e8e1d4]">
                <input
                    type="number"
...
export function InventoryQrGenerator({ initialItems }: { initialItems?: BahamaInventoryV2Item[] } = {}) {
    ...
    const [labelPreset, setLabelPreset] = useState('70x50');
    ...
    const updateLayout = (field: keyof QrLabelLayout, value: number) => {
        if (field === 'widthMm' || field === 'heightMm') setLabelPreset('custom');
        setLayout((current) => ({ ...current, [field]: value }));
    };

    const applyLabelPreset = (value: string) => {
        setLabelPreset(value);
        if (value === '70x50') setLayout((current) => ({ ...current, widthMm: 70, heightMm: 50 }));
        if (value === '90x50') setLayout((current) => ({ ...current, widthMm: 90, heightMm: 50 }));
    };
    ...
    return (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0c111b] text-slate-100 shadow-2xl">
            ...
            <div className="grid min-h-[650px] xl:grid-cols-[minmax(0,1fr)_390px]">
                ...
                <aside className="bg-[#0f1520] p-5 md:p-6">
                    <div className="xl:sticky xl:top-5">
                        <div className="flex items-center justify-between">
                            <h2 className="m-0 text-lg font-semibold">Etikettlayout (A4)</h2>
                            <button type="button" onClick={() => { setLayout(DEFAULT_QR_LABEL_LAYOUT); setLabelPreset('70x50'); }} className="text-xs font-semibold text-[#d4c8b4] hover:text-white">Återställ</button>
                        </div>
                        <label className="mt-4 grid gap-1.5 text-xs font-semibold text-slate-400">
                            Standard
                            <select value={labelPreset} onChange={(event) => applyLabelPreset(event.target.value)} className="rounded-lg border border-white/10 bg-[#111722] px-3 py-2.5 text-sm text-slate-200 outline-none focus:border-[#e8e1d4]">
