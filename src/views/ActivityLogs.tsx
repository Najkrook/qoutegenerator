...
type ActivityLogsPageState = PaginatedLogPageState<ActivityLogRow> & {
    pageStarts: LogCursor[];
    error: boolean;
};
...
function createInitialPageState(): ActivityLogsPageState {
    return {
        loading: false,
        pageIndex: 0,
        pageStarts: [null],
...
function createInitialFilters(): ActivityLogFilters {
    return {
        fromDate: '',
        toDate: '',
        system: '',
...
export function hasActiveActivityFilters(filters: Partial<ActivityLogFilters> = {}) {
    return Object.values(filters).some((value) => String(value || '').trim() !== '');
}

export function getActivityLogsEmptyStateMessage({
    loading = false,
    hasError = false,
...
export function ActivityLogs({ onBack }: ActivityLogsProps) {
    ...
    const [pageState, setPageState] = useState<ActivityLogsPageState>(createInitialPageState);
    const [filters, setFilters] = useState<ActivityLogFilters>(createInitialFilters);
    ...
    const matchesFilters = useCallback((row: ActivityLogRow, activeFilters: ReturnType<typeof getActiveFilters>) => {
    ...
    const fetchLogsBatch = useCallback(async (startCursor: LogCursor) => {
        ...
        const constraints: QueryConstraint[] = [orderBy('createdAt', 'desc'), limit(QUERY_BATCH_SIZE)];
        if (startCursor) {
    ...
    const handleFilterChange = (field: keyof ActivityLogFilters) => (event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
                        ...
                        <tbody className="text-sm divide-y divide-panel-border">
                            {pageState.rows.length === 0 ? (
                                ...
                                pageState.rows.map((row) => {
                                    ...
                                    const referenceValue = typeof row.metadata.reference === 'string' ? row.metadata.reference : '';
                                    const targetValue = referenceValue || (row.targetId && row.targetId !== '-'
