                ...
                <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                        <tr className="bg-black/20 text-[10px] uppercase font-bold text-text-secondary tracking-wider">
                            ...
                            <th className="p-4 border-b border-panel-border text-right">Ert Pris</th>
                            ...
                            <th className="p-4 border-b border-panel-border text-right">Rabatt %</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-panel-border/50">
                        {totals.map((row, idx) => (
                            ...
                            >
                                ...
                                <td className="p-3 text-sm text-right text-primary font-bold">{fmt(row.net)} SEK</td>
                                <td className="p-3 text-sm text-center">{row.qty}</td>
                                <td className="p-3 text-sm text-right text-text-secondary">{fmt(row.gross)} SEK</td>
                                <td className="p-3 text-sm text-right text-danger">-{row.discountPct}%</td>
                            </tr>
                        ))}
                        ...
                        {globalDiscountAmt > 0 && (
                            <tr className="bg-black/10 italic text-text-secondary">
                                ...
                                <td className="p-3 text-sm text-right text-danger font-bold">-{fmt(globalDiscountAmt)} SEK</td>
                                <td colSpan="3"></td>
                            </tr>
                        )}
                    </tbody>
                    <tfoot className="bg-black/30 border-t-2 border-panel-border">
                        <tr>
                            ...
                            <td className="p-4 text-right font-bold text-primary">{fmt(finalTotalSek)} SEK</td>
                            <td colSpan="2" className="p-4 text-right text-xs uppercase font-bold text-text-secondary">Brutto: {fmt(grossTotalSek)} SEK</td>
                            <td className="p-4 text-right text-danger font-bold text-xs">Total Rabatt: -{fmt(totalDiscountSek)} SEK</td>
                        </tr>
                        {state.includesVat && (
                            <>
                                <tr className="border-t border-panel-border/30">
                                    ...
                                    <td className="p-2 text-right font-bold text-text-secondary">{fmt(vatAmount)} SEK</td>
                                    <td colSpan="3"></td>
                                </tr>
                                <tr className="bg-primary/5">
                                    <td colSpan="2" className="p-4 text-right text-lg uppercase font-black text-white">TOTALT ATT BETALA (inkl. moms)</td>
                                    <td className="p-4 text-right font-black text-2xl text-primary">{fmt(totalWithVat)} SEK</td>
                                    <td colSpan="3"></td>
                                </tr>
                            </>
                        )}
                        {!state.includesVat && (
                            <tr className="bg-primary/5">
                                <td colSpan="2" className="p-4 text-right text-lg uppercase font-black text-white">TOTALBELOPP (exkl. moms)</td>
                                <td className="p-4 text-right font-black text-2xl text-primary">{fmt(finalTotalSek)} SEK</td>
                                <td colSpan="3"></td>
                            </tr>
                        )}
                    </tfoot>
