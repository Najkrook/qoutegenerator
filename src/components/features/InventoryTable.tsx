...
import type { BahamaInventoryItem, InventoryTableProps } from '../../types/contracts';

function getGrenstallPrefix(id: string) {
...
function getRowStatus(comment: string) {
...
function normalizeSizeValue(size: unknown) {
    return String(size || '').trim();
}

function isUnknownSizeValue(size: unknown) {
...
export function InventoryTable({ items, searchTerm, onAddToBasket, onEdit, onDelete }: InventoryTableProps) {
    const lowerTerm = (searchTerm || '').toLowerCase();
    const [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>({});
    const [selectedSizes, setSelectedSizes] = useState<Set<string>>(new Set());
    const [isSizeFilterOpen, setIsSizeFilterOpen] = useState(false);
    const sizeFilterRef = useRef<HTMLTableCellElement | null>(null);
    ...
    const searchedItems = useMemo(() => {
        const next = items.filter((item) => {
            ...
            return fields.some((field) => String(field || '').toLowerCase().includes(lowerTerm));
        });

        next.sort((a, b) => String(a.ID || '').localeCompare(String(b.ID || ''), undefined, { numeric: true, sensitivity: 'base' }));
        return next;
    }, [items, lowerTerm]);
    ...
    const groupedRows = useMemo(() => {
        const groups: Array<{ prefix: string; items: BahamaInventoryItem[] }> = [];
        const byPrefix = new Map<string, { prefix: string; items: BahamaInventoryItem[] }>();

        filtered.forEach((item) => {
            const prefix = getGrenstallPrefix(String(item.ID || '-'));
    ...
    const indexByItemRef = useMemo(() => {
        const indexMap = new Map<BahamaInventoryItem, number>();
    ...
    useEffect(() => {
        setSelectedSizes((prev) => {
            ...
            const next = new Set([...prev].filter((size) => allowed.has(size)));
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (sizeFilterRef.current && target instanceof Node && !sizeFilterRef.current.contains(target)) {
    ...
    const handleActionClick = (callback: () => void) => (event: ReactMouseEvent<HTMLButtonElement>) => {
                                ...
                                <React.Fragment key={`group-${group.prefix}`}>
                                    ...
                                    {!isCollapsed &&
                                        group.items.map((item) => {
                                            ...
                                            const { isReserved, isUsed, isMissingParts } = getRowStatus(String(item.Kommentar || ''));
                                            ...
                                            const actualIndex = indexByItemRef.get(item) ?? -1;
                                            ...
                                            return (
                                                <tr key={`${id}-${actualIndex}-${group.prefix}`} className={rowClasses}>
                                                    ...
                                                    <td className="p-3 text-text-primary whitespace-nowrap">{String(item.TYP || 'BA')}</td>
                                                    <td className="p-3 text-text-primary whitespace-nowrap">{String(item.STORLEK || 'Okänd')}</td>
                                                    <td className="p-3 text-text-primary max-w-[400px]" style={{ whiteSpace: 'normal' }}>
                                                        {String(item.BESKRIVNING || '')}
                                                    </td>
                                                    <td className="p-3 text-text-secondary italic">{String(item.Kommentar || '')}</td>
                                                    <td className="p-3 whitespace-nowrap">
                                                        <div className="flex gap-2 justify-center">
                                                            <button
                                                                onClick={handleActionClick(() => onAddToBasket(item))}
